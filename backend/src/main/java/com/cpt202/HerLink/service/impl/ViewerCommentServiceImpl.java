...
public class ViewerCommentServiceImpl implements ViewerCommentService {
    ...
    public void deleteComment(Long currentUserId, Long resourceId, Long commentId) {
        ...
        if (commentId == null) {
            throw AppException.badRequest("Comment id is required.");
        }

        Comment comment = commentMapper.selectById(commentId);
        if (comment == null) {
            throw AppException.notFound("Comment does not exist.");
        }

        if (!resourceId.equals(comment.getResourceId())) {
            throw AppException.badRequest("Comment does not belong to this resource.");
        }

        CurrentUserVO currentUser = userAccessService.getCurrentUserById(currentUserId);
        boolean isAdmin = UserRoleEnum.ADMINISTRATOR.matches(currentUser.getRole());
        boolean isOwner = currentUserId.equals(comment.getUserId());

        if (!isAdmin && !isOwner) {
            throw AppException.forbidden("You do not have permission to delete this comment.");
        }
