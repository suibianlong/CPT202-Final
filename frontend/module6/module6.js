...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (page === "viewer-feedback") {
        await initViewerFeedbackPage();
    }
});
...
async function initHeritageViewerPage() {
    ...
    bindViewerNavigationButton("feedbackBtn", "./viewer-feedback.html");
    bindViewerSearchControls();

    try {
        await ensureViewerAuthenticated();
        await loadViewerCategoryOptions();
        await loadViewerResourceTypeOptions();
...
async function initViewerFeedbackPage() {
    bindViewerDetailForms();
    ...
    try {
        ...
        handleViewerError(error, "Unable to load feedback information right now.");
    }
}

function bindViewerNavigationButton(buttonId, href) {
...
function bindViewerDetailForms() {
    ...
    if (commentList) {
        commentList.addEventListener("click", async event => {
            const deleteButton = event.target.closest(".viewer-comment-delete-btn");
            if (!deleteButton) return;

            const commentId = deleteButton.dataset.commentId;
            if (!commentId) return;

            await deleteViewerComment(commentId);
        });
    }
...
async function loadViewerResourceTypeOptions() {
    viewerResourceTypeOptions = await viewerRequestJson(`${VIEWER_API_BASE}/resource-type-options`, { method: "GET" });
    ...
    normalizeViewerResourceTypeOptions(viewerResourceTypeOptions).forEach(option => {
        const optionElement = document.createElement("option");
        optionElement.value = normalizeViewerResourceTypeValue(option.name);
        optionElement.textContent = formatViewerResourceType(option.name);
        select.appendChild(optionElement);
    });

    if (currentValue) {
        select.value = normalizeViewerResourceTypeValue(currentValue);
    }
}
...
function renderViewerComments(comments) {
    ...
    commentList.innerHTML = comments.map(comment => {
        ...
        const deleteButtonHtml = canDelete
            ? `<button type="button" class="btn btn-secondary viewer-comment-delete-btn" data-comment-id="${escapeViewerHtml(comment.id)}">Delete</button>`
            : "";
        ...
        return `
            <article class="viewer-comment-item">
                ...
                ${deleteButtonHtml ? `<div class="viewer-comment-actions">${deleteButtonHtml}</div>` : ""}
            </article>
        `;
    }).join("");
}
...
async function deleteViewerComment(commentId) {
    const resourceId = getViewerResourceIdFromQuery();
    if (!resourceId || !commentId) return;

    try {
        await viewerRequestJson(`${VIEWER_API_BASE}/${resourceId}/comments/${commentId}`, {
            method: "DELETE"
        });

        showViewerToast("Comment deleted successfully.");
        await loadViewerComments();
    } catch (error) {
        if (error?.status === 401) {
            redirectViewerToLogin();
        ...
        showViewerToast(error.message || "Unable to delete the comment.");
    }
}

async function loadViewerFeedbackHistory() {
    const requestUrl = isViewerAdmin()
        ? `${VIEWER_FEEDBACK_API_BASE}/all`
        : `${VIEWER_FEEDBACK_API_BASE}/mine`;
    const feedbackList = await viewerRequestJson(requestUrl, { method: "GET" });
...
function handleViewerError(error, fallbackMessage) {
    ...
    if (feedbackList) {
        feedbackList.innerHTML = `<div class="viewer-error">${escapeViewerHtml(message)}</div>`;
    }
...
function canViewerDeleteComment(comment) {
    const isOwner = viewerCurrentUser && String(viewerCurrentUser.userId) === String(comment.userId);
    return Boolean(isOwner || isViewerAdmin());
}

function isViewerAdmin() {
    const role = String(viewerCurrentUser?.role || "").trim().toUpperCase();
...
function normalizeViewerResourceTypeValue(value) {
    return normalizeViewerResourceType(value);
}
