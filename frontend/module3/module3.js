...
async function initResourceEditPage() {
    ...
    await loadResourceTypeSelectOptions();
...
function applyEditorScenery() {
    ...
    if (scrollScene) {
        scrollScene.style.removeProperty("--scroll-scene-image");
    }
    ...
    panels.forEach(panel => panel.style.removeProperty("--panel-bg-image"));
}
...
function applyMyResourcesScenery() {
...
async function loadResourceTypeSelectOptions() {
    const mirrorSelect = document.getElementById("resourceTypeMirror");
    const primarySelect = document.getElementById("resourceType");
    if (!mirrorSelect || !primarySelect) return;

    try {
        const options = normalizeResourceTypeOptions(await requestJson(`${API_BASE}/resource-type-options`, { method: "GET" }));
        if (!options.length) {
            throw new Error("No active resource types are available.");
        }

        resourceTypeOptionCache = options;
        renderResourceTypeSelectOptions(mirrorSelect, options, "Select type");
        ensureResourceTypeOption(primarySelect, primarySelect.value);
        syncResourceTypeMirror();
        mirrorSelect.disabled = false;
    } catch (error) {
        resourceTypeOptionCache = [];
        renderResourceTypeSelectOptions(mirrorSelect, [], "Resource types unavailable");
        primarySelect.value = "";
        mirrorSelect.value = "";
        mirrorSelect.disabled = true;
        setActionDisabled("metadataSaveBtn", true);
        setActionDisabled("submitReviewBtn", true);
        showEditorAlert("Resource types failed to load. Metadata save and submit are temporarily disabled.");
        showToast(error.message || "Failed to load resource types.");
    }
}
...
async function openHistoryModal(resourceId) {
    ...
    const modalDialog = historyModal.querySelector(".heritage-history-modal-dialog");
    if (modalDialog) {
...
function fillEditor(detail) {
    ...
    ensureResourceTypeOption(document.getElementById("resourceTypeMirror"), detail.resourceType);
...
async function createDraftFromDirectEditorEntry() {
    ...
    try {
        ...
        const newResourceId = draft?.id ?? draft?.resourceId;
        if (!newResourceId) {
...
function renderResourceTypeSelectOptions(selectElement, options, placeholderText) {
    const currentValue = selectElement.value;
    const html = [`<option value="">${placeholderText}</option>`];

    normalizeResourceTypeOptions(options).forEach(option => {
        const optionValue = normalizeResourceTypeValue(option.name);
        html.push(`<option value="${escapeHtml(optionValue)}">${escapeHtml(formatResourceType(option.name))}</option>`);
    });

    selectElement.innerHTML = html.join("");

    if (currentValue) {
        ensureResourceTypeOption(selectElement, currentValue);
        selectElement.value = normalizeResourceTypeValue(currentValue);
    }
}

function ensureResourceTypeOption(selectElement, value) {
    if (!selectElement) return;

    const normalizedValue = normalizeResourceTypeValue(value);
    ...
    optionElement.textContent = formatResourceType(normalizedValue);
...
function normalizeResourceTypeOptions(options) {
    if (!Array.isArray(options)) {
        return [];
    }

    return options
        .map(option => normalizeResourceTypeOption(option))
        .filter(Boolean);
}

function normalizeResourceTypeOption(option) {
    if (option === null || option === undefined) {
        return null;
    }
    ...
    return {
        id: option.id ?? option.resourceTypeId ?? null,
        name
    };
}
